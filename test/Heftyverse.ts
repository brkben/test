  ...
  describe("Single Market place: 721 NFT", async () => {
    ...
    describe("Template 1155 test cases", async () => {
      ...
      it("Admin set as Zero address",async()=>{
        await expect(template1155.connect(signers[1]).setAdmin(ethers.constants.AddressZero)).to.be.revertedWith("ZA");
        })          
      ...
      await expect(template1155.connect(signers[1]).redeem(voucher2,signers[2].address,expandTo6Decimals(5))).to.be.revertedWith("IS");
      });
    ...
    it("non token owner caller for redeem", async () => {
      ...
      const voucher3= await Template1155Voucher3.createVoucher(template1155.address,1,expandTo6Decimals(10),1,1,"TestURI",true,signers[2].address,expandTo6Decimals(0));    
      // await template1155.isApprovedForAll(template1155
      await expect(template1155.connect(signers[2]).redeem(voucher3,signers[2].address,1)).to.be.revertedWith("ERC1155:caller is not token owner nor approved");
    });
    
    it("Redeem 1155 with different amounts ", async () => {
      const Template1155Voucher3= await new template1155Voucher({_contract:template1155, _signer:signers[1]});
      const voucher3= await Template1155Voucher3.createVoucher(template1155.address,1,expandTo6Decimals(10),3,1,"TestURI",true,ethers.constants.AddressZero,expandTo6Decimals(0));    
      await template1155.connect(signers[1]).redeem(voucher3,signers[2].address,2);
      await template1155.connect(signers[1]).redeem(voucher3,signers[3].address,1);
    });
    it("safeBatchTransferFrom ", async () => {
      const Template1155Voucher= await new template1155Voucher({_contract:template1155, _signer:signers[1]});
      const voucher= await Template1155Voucher.createVoucher(template1155.address,1,expandTo6Decimals(10),2,1,"TestURI",true,ethers.constants.AddressZero,expandTo6Decimals(0));
      const voucher2= await Template1155Voucher.createVoucher(template1155.address,2,expandTo6Decimals(10),3,1,"TestURI",true,ethers.constants.AddressZero,expandTo6Decimals(0));
    ...
    it("safeBatchTransferFrom:non token owner caller", async () => {
      ...
      await template1155.connect(signers[1]).redeem(voucher,signers[2].address,1);
      await template1155.connect(signers[1]).redeem(voucher2,signers[2].address,1);
      await expect(template1155.connect(signers[3]).safeBatchTransferFrom(signers[2].address,signers[3].address,[1,2],[1,1],[])).to.be.revertedWith("ERC1155: not approved");
          ...
          it("custodial to custodial secondary buy : mismatched addresses",async()=>{
            ...
            it("custodial to custodial primary buy: counters mismatched",async()=>{
              ...
              it("custodial to custodial primary buy: mismatched amounts",async()=>{
                ...
                const sellerVoucher = new SellerVoucher({_contract:singleMarketplace , _signer:owner});
                const VoucherSell = await sellerVoucher.createVoucher1155(Tseries, owner.address,1,1,expandTo6Decimals(10),1,true,true);
                const buyerVoucher = new BuyerVoucher({_contract:singleMarketplace , _signer:signers[6]});
                const Voucherbuy = await buyerVoucher.createVoucher1155(Tseries, signers[6].address,1,2,expandTo6Decimals(10),1,true);
                const TemplateVoucher = await new LazyMinting({
                    _contract: TRS,
                ...
                await usdt.connect(owner).approve(singleMarketplace.address,expandTo6Decimals(1000));
                //console.log("NFT balance of first buyer before buy:",await TRS.balanceOf(signers[6].address,1));
                await expect(singleMarketplace.Buy(Voucherbuy, VoucherSell, voucher,voucherNFT,true)).to.be.revertedWith("AMI");
              })    
